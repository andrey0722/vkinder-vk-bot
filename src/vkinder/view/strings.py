...
class MenuTokenStr(enum.StrEnum):
    ...
    AUTH_BEGIN = '📝 Авторизоваться'
    AUTH_FINISHED = '✅ Я завершил авторизацию'
...
HELP_MAP: Final[dict[MenuTokenStr, str]] = {
    ...
    MenuTokenStr.AUTH_BEGIN: 'начать процесс авторизации в другом окне',
    MenuTokenStr.AUTH_FINISHED: 'нужно нажать после завершения авторизации',
}
...
class Strings(enum.StrEnum):
    ...
    AUTH_REQUIRED = (
        f'Для поиска анкет и просмотра фотографий необходимо дать '
        f'разрешения боту.\n'
        f'1. Нажми "{MenuTokenStr.AUTH_BEGIN}" и подтверди доступ\n'
        f'2. После авторизации нажми "{MenuTokenStr.AUTH_FINISHED}"\n'
        f'3. PROFIT'
    )
    AUTH_NOT_COMPLETED = '⚠ Авторизация не завершена.'
